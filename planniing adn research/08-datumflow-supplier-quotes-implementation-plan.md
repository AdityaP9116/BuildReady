...
**Current push checkpoint:** an operator-assisted local mesh/solve/CSV path and human manufacturing-input review are implemented, but live acceptance and unified live-result consumption are not complete. The detailed [unfinished-work note](../docs/unfinished-work.md) supersedes older implementation-status statements below; [the partner runbook](../docs/partner-demo-runbook.md) describes the opt-in execution controls. This does not close any numerical or end-to-end gate.

**2026-09-03 demo scope update (supersedes earlier readiness statements below):** the user requested fictional supplier placeholders for the demonstration and delegated selection of an illustrative stress setup. Real supplier acquisition is therefore not a demo prerequisite; the genuine-evidence track remains separate and incomplete. The current Onshape bracket has been exported locally from an existing immutable version, with its actual STEP digest and final-solid face inventory retained privately for seven days. An illustrative 6061-T6 / fixed mounting holes / 100 N toward-base draft is bound to that receipt; it is not engineering approval or authority to consume compute. SimScale import/topology mapping, worker/results integration, free entitlement, bounded transfer/run approval and numerical acceptance remain required. A preferred-supplier directory is planned, not implemented; manual private quote entry already exists. See document 09 and local setup for the current evidence.
