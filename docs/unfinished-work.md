...
3. **Reconciliation acceptance:** operator-assisted lost-response reconciliation is implemented for supported SimScale import, mesh/simulation/run creation, start and cancellation stages. It uses provider reads, frozen-input comparisons and immutable reviewer evidence, never replaying the original write. Real-account acceptance remains unverified; legacy uncertain operations lacking retained request data stay blocked, and import geometry/storage linkage remains explicitly operator-attested.
