...
2. **Geometry recognition:** manufacturing inputs and SimScale face mappings currently require explicit human entry/review. The illustrative setup builder is limited to the inspected bracket. General-purpose geometry-to-role mapping is not implemented. Human manufacturing measurement reviews are now durably stored for seven days and bound immutably to the exact source snapshot; they remain manual evidence and never grant production approval. Reviewed SimScale mappings are now durable and bound to an exact mesh level and provider run, but the operator still has to identify the imported entities.
3. **Reconciliation:** all current provider operations now share an explicit durable lifecycle, and the SimScale journal exposes frozen request hashes, attempt counts, timestamps, bounded remote IDs and restart-stable retry/reconciliation flags. Uncertain external writes are safely blocked from replay, but recovery of lost creation receipts is not automated. Add a verified reconciliation path and explicit operator resolution. Do not bypass this by deleting journal rows or creating duplicate operations.
