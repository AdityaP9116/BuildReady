...
- `ONSHAPE_NO_VARIABLES`: the model read lacks named variables for this application's parameter-based screening. This does not prove the underlying solid is invalid. Do not alter real geometry just to satisfy demonstration checks.
...
Choose and freeze an immutable Onshape version, microversion, default configuration and single part. Verify the exact STEP bytes/provenance, reviewed material/support/load setup, topology mapping and provider read-back. Complete the live worker and numerical verification machinery, then obtain human approval for the specific public benchmark transfer and a bounded free-run allowance. The working API key and viewer context do not satisfy those gates. Keep `SIMULATION_PROVIDER=recorded` or `disabled` until then.
